    ...
    def getDatasetFeatures(self, dataset_id, bbox, crs_epsg, limit=None):
        with openapi_client.ApiClient(self.configuration) as api_client:
            try:
                ...
                if limit:
                    api_response = features_api_instance.get_dataset_features(self.x_client_product_version, dataset_id, references='none', locking='', limit=limit, bbox=','.join(map(str, bbox)), crs_epsg=crs_epsg)
                else:
                    api_response = features_api_instance.get_dataset_features(self.x_client_product_version, dataset_id, references='none', locking='', bbox=','.join(map(str, bbox)), crs_epsg=crs_epsg)
                return api_response
            except ApiException as e:
                print("Exception when calling FeaturesApi->get_dataset_feature: %s\n" % e)
